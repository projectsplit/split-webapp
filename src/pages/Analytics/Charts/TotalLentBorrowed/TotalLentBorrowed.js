...
export function TotalLentBorrowed({ selectedCycle, selectedYear, currentWeekIndex, monthsAndDaysArrays, cyclehaschanged, allWeeksPerYear, menu, selectedTimeCycleIndex, currency, endDate, startDate, backendData, isSuccess, timeZone, }) {
    ...
    const totalLent = totalLentBorrowed?.totalLent === undefined
        ? []
        : totalLentBorrowed.totalLent;
    const totalBorrowed = totalLentBorrowed?.totalBorrowed === undefined
        ? []
    ...
    const pointRadius = [];
    const hitRadius = []; //determines which cicles will be highlited on hover.
    const pointBackgroundColorTotalLent = [];
    const pointBackgroundColorTotalLentTotalBorrowed = [];
    totalLentExt.map((dp, indx) => {
        if (indx === 0 ||
            ...
            if (enhancedDatesToNumbers[indx] === 15 &&
    ...
    const options = getChartOptions(isSuccess, totalLentExt, totalBorrowedExt, selectedCycle.value, labels, enhancedDatesToNumbers, selectedYear.value, selectedTimeCycleIndex.value, currentWeekIndex, hitRadius, fractalFactor, currency);
    const data = getData(totalLentExt, totalBorrowedExt, labels, pointRadius, pointBackgroundColorTotalLent, pointBackgroundColorTotalLentTotalBorrowed);
    return (_jsxs(StyledTotalLentBorrowed, { children: [_jsx(Line, { options: options, data: data, plugins: [noData, ChartDataLabels] }), _jsx("div", { className: "periodOptions", children: _jsx(Carousel, { carouselItems: getCarouselItemsBasedOnCycle(selectedCycle.value, months, monthsAndDaysArrays), selectedTimeCycleIndex: selectedTimeCycleIndex, selectedCycle: selectedCycle, cyclehaschanged: cyclehaschanged, menu: menu, selectedYear: selectedYear }) })] }));
}
