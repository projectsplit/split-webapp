export const getData = (totalLentExt, totalBorrowedExt, labels, pointRadius, pointBackgroundColorTotalLent, pointBackgroundColorTotalLentTotalBorrowed) => {
    const paintBackgroundUnderLines = (ctx) => {
        if (totalLentExt !== undefined && totalBorrowedExt !== undefined)
            if (totalBorrowedExt[ctx.p0DataIndex] > totalLentExt[ctx.p0DataIndex] ||
                totalBorrowedExt[ctx.p1DataIndex] > totalLentExt[ctx.p1DataIndex]) {
                return 'rgba(255, 74, 94, 0.23)'; //red
            }
            else
                return 'rgba(34, 135, 29,0.2)'; //green
    };
    return {
        labels: labels,
        datasets: [
            {
                ...
                stepped: false,
                segment: {
                    backgroundColor: (ctx) => paintBackgroundUnderLines(ctx),
                },
            },
            {
