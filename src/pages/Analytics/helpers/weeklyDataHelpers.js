...
export const getMonthWeek = (year, week) => {
...
export const generateAllWeeksPerYear = (year) => {
    const allWeeks = [];
...
export function findIndexForCurrentDate(dateArray, date) {
    let foundIndex = -1;
...
export const weeksToDateString = (weeks) => {
    return weeks.map((week) => week.map((day) => day.toDateString()));
};
export const dateStringToMonthAndDay = (weeks) => {
    return weeks.map((week) => keepMonthAndNumberOfDay(week));
};
export function keepMonthAndNumberOfDay(dateArray) {
...
export const numberOfweeksInYear = (year) => {
