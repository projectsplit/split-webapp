...
export const buildStartAndEndDates = (
  ...
  switch (cycle) {
    ...
    case Frequency.Weekly:
      startDate = formatDateIntoYMD(
        getDateFromWeeksArray(allWeeksPerYear, selectedTimeCycleIndex, 0),
        timeZone
      );
      endDate = formatDateIntoYMD(
    ...
    case Frequency.Annually:
      ...
      startDate = formatDateIntoYMD(
        new Date(getWeekDates(selectedYear, startWeekIndex)[0]),
        timeZone
      );
      endDate = formatDateIntoYMD(
        new Date(
          getWeekDates(selectedYear, endWeekIndex)[
            getWeekDates(selectedYear, endWeekIndex).length - 1
          ]
        ),
        timeZone
      );
