...
const _displayedAmount = signal('');
const _openCalendar = signal(false);
const _openCustomDateCalendar = signal(false);
const _startDate = signal('');
const _endDate = signal('');
const _pickingTarget = signal(null);
const _calendarDay = signal('');
const _budgetFrequency = signal(Frequency.Monthly);
const _hasSwitchedBudgetType = signal(false);
...
const _targetGroupIds = signal([]);
const _allGroupsSelected = signal(true);
const _serverErrors = signal([]);
const createBudgetStore = create()((set, get) => ({
    amount: '',
    description: '',
    currencySymbol: '',
    ...
    validateForm: (step, options = { showErrors: true }) => {
        const { amount, description, scopeState, budgetFrequency, targetGroupIds, startDate, endDate, calendarDay, } = get();
        const { amountError, descriptionError, spendingCycleError, scopeError, commencementDayError, } = validateBudgetForm(amount, description, scopeState, step, budgetFrequency, targetGroupIds, startDate, endDate, calendarDay);
    ...
    submitBudget: async ({ createBudgetMutation, updateBudgetMutation, menu, step, activate }) => {
        ...
        const { amount, description, budgetFrequency, currencySymbol, scopeState, allGroupsSelected, targetGroupIds, calendarDay, startDate, endDate, displayedAmount, openCalendar, hasSwitchedBudgetType, } = get();
    ...
    resetForm: () => {
        const { displayedAmount, openCalendar, openCustomDateCalendar, startDate, endDate, pickingTarget, calendarDay, budgetFrequency, hasSwitchedBudgetType, scopeState, targetGroupIds, allGroupsSelected, serverErrors, } = get();
        displayedAmount.value = '';
        openCalendar.value = false;
    ...
    populateForm: (budget, currencySymbol) => {
        const { displayedAmount, openCalendar, openCustomDateCalendar, startDate, endDate, pickingTarget, calendarDay, budgetFrequency, hasSwitchedBudgetType, scopeState, targetGroupIds, allGroupsSelected, serverErrors, } = get();
