...
export default function CreateBudget() {
    ...
    const makeBudgetActiveMenu = useSignal(null);
    const scopeMenu = useSignal(null);
    ...
    const { mutateAsync: createBudget, isPending: isCreatePending } = useCreateBudget(navigate, data.serverErrors, menu);
    const { mutateAsync: updateBudget, isPending: isUpdatePending } = useEditBudget(navigate, data.serverErrors, menu);
    const isPending = isCreatePending || isUpdatePending;
    const location = useLocation();
    const populatedKeyRef = useRef(null);
    useEffect(() => {
    ...
    const handleInputChangeCallback = useCallback((e) => {
        handleInputChange(e, data.currencySymbol, data.displayedAmount, actions.setAmount);
    ...
    const handleNext = () => {
        ...
        const hasStep1Errors = !!errors.amountError ||
    ...
    return (_jsxs(StyledCreateBudget, { children: [_jsx(TopBarWithBackButton, { header: data.isEditMode ? "Edit Budget" : "Create Budget", onClick: () => {
                ...
                } }), _jsx(BackAndForthAnimation, { firstChild: _jsx(FirstPage, { data: data, actions: actions, menu: menu, handleInputChangeCallback: handleInputChangeCallback, timeZoneId: timeZoneId }), secondChild: _jsx(SecondPage, { data: data, actions: actions, scopeMenu: scopeMenu }), currentStep: data.currentStep, animDirection: animDirection }), _jsx("div", { className: "submitButton", children: _jsx(MyButton, { fontSize: "16", onClick: data.currentStep === 1 ? handleNext : submitBudget, isLoading: isPending, children: data.currentStep === 1 ? 'Next' : data.isEditMode ? 'Update Budget' : 'Submit Budget' }) }), _jsx(MenuAnimationBackground, { menu: menu }), _jsx(MenuAnimationBackground, { menu: makeBudgetActiveMenu }), scopeMenu.value === 'scopeSelector' && (_jsx(ScopeSelectionMenu, { menu: scopeMenu, scopeState: data.scopeState, targetGroupIds: data.targetGroupIds, allGroupsSelected: data.allGroupsSelected })), _jsx(CreateBudgetConfirmationAnimation, { menu: menu, submitBudget: async () => {
                    await submitBudget();
                } }), _jsx(InfoBoxAnimation, { menu: menu, children: _jsx(SpendingCycleInfo, { menu: menu }) }), _jsx(MakeBudgetActiveMenuAnimation, { menu: makeBudgetActiveMenu, hasActiveBudgetData: !!activeBudgetData, hasInactiveBudgetData: !!inactiveBudgetsData && inactiveBudgetsData.budgets.length > 0, onConfirm: (activate) => {
                ...
                } }), _jsx(CurrencyOptionsAnimation, { currencyMenu: menu, clickHandler: handldeCurrencyOptionsClick, selectedCurrency: data.currencySymbol })] }));
}
