...
export const useExpenseTotals = (
  ...
  const totals = useMemo(() => {
    const totalSpentByCurrency: Record<
      string,
      Record<string, number>
    > = debts?.totalSpent ?? {};
    const convertedTotalSpent: Record<string, number> =
      debts?.convertedTotalSpent ?? {};

    const groupTotalsByCurrency = getAllCurrencyTotals(totalSpentByCurrency);
    const userTotalsByCurrency =
      mode === Mode.Group
        ? getCurrencyValues(totalSpentByCurrency, userMemberId)
        : getCurrencyValues(totalSpentByCurrency, userInfo?.userId);

    const totalFromAllExpensesConverted = Object.values(
      convertedTotalSpent
