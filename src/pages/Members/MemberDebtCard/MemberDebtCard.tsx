...
function MemberDebtCard({
  ...
  return (
    <StyledMemberDebtCard isGuest={isGuest} isLoggedUser={isLoggedUser}>
      ...
      {(memberIsOwedItems.length > 1 || memberOwesItems.length > 1) && (
        <div className="debtTree">
          {memberIsOwedItems.length > 1 && <Tree items={memberIsOwedItems} />}
          {memberOwesItems.length > 1 && <Tree items={memberOwesItems} />}
        </div>
      )}
