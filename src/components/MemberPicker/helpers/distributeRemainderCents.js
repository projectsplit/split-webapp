export function distributeRemainderCents(decimalDigits, totalAmount, screenArray, synchronizedFormMembers) {
    ...
    const splitOfLockeAmountsInCents = lockedPercentages.map((s) => Math.round((s * totalCents) / 100), 0);
    const remainderCents = totalCents -
        splitOfUnlockedAmountsInCents.reduce((a, b) => a + b, 0) -
        splitOfLockeAmountsInCents.reduce((a, b) => a + b, 0);
    if (remainderCents === 0) {
        const adjustedLockedToOriginalAmount = splitOfLockeAmountsInCents.map((s) => (s / multiplier).toFixed(decimalDigits));
        const adjustedUnlockedToOriginalAmount = splitOfUnlockedAmountsInCents.map((s) => (s / multiplier).toFixed(decimalDigits));
        return { adjustedUnlockedToOriginalAmount, adjustedLockedToOriginalAmount };
    }
    const adjustArray = (arr, remainder) => {
        const adjusted = [...arr];
    ...
    if (splitOfLockeAmountsInCents.length > 0) {
        ...
        adjustedUnlocked = adjustArray(splitOfUnlockedAmountsInCents, remainderCents);
    }
    const newTotal = adjustedUnlocked.reduce((a, b) => a + b, 0) +
        adjustedLocked.reduce((a, b) => a + b, 0);
    if (newTotal !== totalCents) {
        console.warn(`Adjustment failed: new total ${newTotal} does not match totalCents ${totalCents}`);
    }
    const adjustedLockedToOriginalAmount = adjustedLocked.map((s) => (s / multiplier).toFixed(decimalDigits));
    const adjustedUnlockedToOriginalAmount = adjustedUnlocked.map((s) => (s / multiplier).toFixed(decimalDigits));
    return {
