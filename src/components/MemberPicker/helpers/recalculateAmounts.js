...
export const recalculateAmounts = (formMembers, totalAmount, decimalDigits, category, ticker, isCreateExpense) => {
    const getCleanActual = (sq) => {
    ...
    const synchronizedFormMembers = formMembers.map((m) => m.locked && m.selected
        ? { ...m, actualAmount: getCleanActual(m.screenQuantity) }
        : m);
    const lockedSelectedMembers = synchronizedFormMembers.filter((m) => m.selected && m.locked);
    const unlockedSelectedMembers = synchronizedFormMembers.filter((m) => m.selected && !m.locked);
    const lockedTotalAmount = lockedSelectedMembers
        .map((m) => Number(m.actualAmount))
        .reduce((total, a) => total + a, 0);
    let screenArray = [];
    let actualAmountsArray = [];
    let actualPercentageSplit = [];
    switch (category.value) {
        case 'Amounts':
            screenArray = split(totalAmount - lockedTotalAmount, unlockedSelectedMembers.length, decimalDigits);
        ...
        case 'Percentages':
            screenArray = split(100 - lockedTotalAmount, unlockedSelectedMembers.length, decimalDigits + 2);
            actualPercentageSplit = split(100 - lockedTotalAmount, unlockedSelectedMembers.length, decimalDigits + 2);
            const { adjustedUnlockedToOriginalAmount: adjustedUnlocked, adjustedLockedToOriginalAmount: adjustedLocked, } = distributeRemainderCents(decimalDigits, totalAmount, actualPercentageSplit, synchronizedFormMembers);
            let unlockedIndex = 0;
            let lockedIndex = 0;
            return synchronizedFormMembers.map((m) => {
                if (m.selected && !m.locked) {
        ...
        case 'Shares':
            const totalShares = synchronizedFormMembers.reduce((sum, member) => sum +
            ...
            const { adjustedToOriginalAmount } = distributeRemainderCentsForShares(decimalDigits, totalAmount, synchronizedFormMembers);
            return synchronizedFormMembers.map((m) => {
                if (m.selected) {
