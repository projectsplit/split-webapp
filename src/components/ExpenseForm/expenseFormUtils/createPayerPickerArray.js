...
export const createPayerPickerArray = (groupMembers, nonGroupUsers, expense, type, isCreateExpense, userId, userMemberId, isnonGroupExpense, currentAmount) => {
    let array = [];
    if (isnonGroupExpense && nonGroupUsers.length > 0) {
        array = nonGroupUsers.map((user) => {
            ...
            const screenQuantity = isPercentageType &&
                expense?.amount &&
                payer?.paymentAmount &&
                !isNaN(expenseAmount) &&
                expenseAmount !== 0
            ...
            return {
                ...
                actualAmount,
                screenQuantity,
                locked: (isNonGroupExpense(expense) &&
                    expense.payers.some((p) => p.userId === user.userId)) ??
                    false,
                name: user.username,
                order: 0,
                selected: (isNonGroupExpense(expense) &&
                    expense.payers.some((p) => p.userId === user.userId)) ??
                    false,
            };
        });
        ...
        array = groupMembers.map((member) => {
            const payer = expense && isGroupExpense(expense)
    ...
    if (isCreateExpense) {
        const selectedId = isnonGroupExpense && nonGroupUsers.length > 0 ? userId : userMemberId;
        array = array.map((m) => {
            ...
            return {
                ...
                selected: isSelected,
                order: 0,
                actualAmount: isSelected && currentAmount ? currentAmount : m.actualAmount,
            };
        });
    }
    return array;
