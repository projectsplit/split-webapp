...
export const validateExpenseState = (amount, participantsCategory, payersCategory, currencySymbol, participantsByCategory, payersByCategory) => {
    let amountErr = '';
    let participantsErr = '';
    let payersErr = '';
    ...
    const isAmountEmptyOrZero = !amountTrimmed || amountTrimmed === '0' || amountTrimmed === '0.';
    ...
    const areParticipantsValid = selectedParticipants.every((p) => p.actualAmount !== 'NaN' && Number(p.actualAmount) > 0);
    const digits = significantDigitsFromTicker(currencySymbol);
    let isParticipantsSumInvalid = false;
    if (digits >= 3) {
        const sum = Number(selectedParticipants
        ...
        const sum = selectedParticipants.reduce((acc, p) => currency(acc).add(p.actualAmount).value, 0);
    ...
    participantsErr = !areParticipantsValid
        ...
        : isParticipantsSumInvalid
            ...
            : '';
    // Payers validation
    const arePayersValid = selectedPayers.every((p) => p.actualAmount !== 'NaN' && Number(p.actualAmount) > 0);
    let isPayersSumInvalid = false;
    if (digits >= 3) {
        const sum = Number(selectedPayers
            .reduce((acc, p) => acc + Number(p.actualAmount), 0)
            .toFixed(digits));
        const target = Number(currentAmount.toFixed(digits));
        ...
        const sum = selectedPayers.reduce((acc, p) => currency(acc).add(p.actualAmount).value, 0);
