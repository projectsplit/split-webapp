...
export const createParticipantPickerArray = (groupMembers, nonGroupUsers, expense, type, isCreateExpense, isnonGroupExpense) => {
    let array = [];
    if (isnonGroupExpense && nonGroupUsers.length > 0) {
        ...
        array = groupMembers.map((member) => {
            const participant = expense && isGroupExpense(expense)
