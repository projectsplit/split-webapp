...
import { generatePickerArrays, validateExpenseState, submitExpenseFromState, } from '../expenseFormUtils';
...
export const useExpenseStore = create()((set, get) => ({
    ...
    showPicker: false,
    participantsCategory: signal('Amounts'),
    payersCategory: signal('Amounts'),
    // ── Simple setters ──────────────────────────────────────────────
    ...
    setExpenseTime: (value) => set((state) => ({
        expenseTime: typeof value === 'function' ? value(state.expenseTime) : value,
    })),
    ...
    setAmountError: (msg) => set({ amountError: msg }),
    setShowAmountError: (show) => set({ showAmountError: show }),
    setParticipantsError: (msgOrUpdater) => set((state) => ({
        participantsError: typeof msgOrUpdater === 'function'
            ? msgOrUpdater(state.participantsError)
            : msgOrUpdater,
    })),
    setPayersError: (msgOrUpdater) => set((state) => ({
        payersError: typeof msgOrUpdater === 'function'
            ? msgOrUpdater(state.payersError)
            : msgOrUpdater,
    })),
    setDescriptionError: (msg) => set({ descriptionError: msg }),
    // ── Safe complex updates ────────────────────────────────────────
    setParticipantsByCategory: (updater) => set((state) => {
        const prevByCategory = state.participantsByCategory;
        const newByCategory = typeof updater === 'function' ? updater(prevByCategory) : updater;
    ...
    setPayersByCategory: (updater) => set((state) => {
    ...
    updateParticipantsInCategory: (category, updater) => set((state) => ({
        participantsByCategory: {
            ...state.participantsByCategory,
            [category]: updater(state.participantsByCategory[category]),
        },
    })),
    updatePayersInCategory: (category, updater) => set((state) => ({
        payersByCategory: {
    ...
    initialize: (config) => {
        const { isCreateExpense, expense, currency, groupMembers, nonGroupUsers, isnonGroupExpense, userInfo, } = config;
        const userMembers = groupMembers
            ?.peek()
            .filter((item) => 'userId' in item);
        const userMemberId = userMembers?.find((m) => m.userId === userInfo?.userId)?.id;
        const initialCurrency = isCreateExpense || !expense ? currency : expense.currency;
        const initialAmount = isCreateExpense || !expense ? '' : expense.amount;
        const initialDescription = isCreateExpense || !expense ? '' : expense.description;
        const initialLabels = isCreateExpense || !expense ? [] : expense.labels;
        const initialExpenseTime = isCreateExpense || !expense
        ...
        const { participantsByCategory, payersByCategory } = generatePickerArrays(groupArr, nonGroupArr, expense, isCreateExpense, userInfo, userMemberId, isNonGroup, initialAmount);
        ...
        for (const cat of ['Amounts', 'Shares', 'Percentages']) {
            finalParticipants[cat] = recalculateAmounts(participantsByCategory[cat], amountNum, decimalDigits, { value: cat }, initialCurrency, isCreateExpense);
            finalPayers[cat] = recalculateAmounts(payersByCategory[cat], amountNum, decimalDigits, { value: cat }, initialCurrency, isCreateExpense);
        }
        set({
    ...
    updateMembers: (config) => {
        const { groupMembers, nonGroupUsers, expense, isCreateExpense, isnonGroupExpense, userInfo, userMemberId, } = config;
        const userMembers = groupMembers
            ?.peek()
            .filter((item) => 'userId' in item);
        let derivedUserMemberId = userMemberId;
        if (!derivedUserMemberId && userMembers) {
            derivedUserMemberId = userMembers.find((m) => m.userId === userInfo?.userId)?.id;
        }
        derivedUserMemberId = userMembers?.find((m) => m.userId === userInfo?.userId)?.id;
        const groupArr = groupMembers?.peek() ?? [];
        const nonGroupArr = nonGroupUsers?.peek() ?? [];
        ...
        const { participantsByCategory, payersByCategory } = generatePickerArrays(groupArr, nonGroupArr, expense, isCreateExpense, userInfo, derivedUserMemberId, isNonGroup, currentAmount);
        const amountNum = Number(currentAmount);
        const decimalDigits = significantDigitsFromTicker(get().currencySymbol);
        const finalParticipants = { ...participantsByCategory };
        const finalPayers = { ...payersByCategory };
        for (const cat of ['Amounts', 'Shares', 'Percentages']) {
            finalParticipants[cat] = recalculateAmounts(participantsByCategory[cat], amountNum, decimalDigits, { value: cat }, get().currencySymbol, isCreateExpense);
            finalPayers[cat] = recalculateAmounts(payersByCategory[cat], amountNum, decimalDigits, { value: cat }, get().currencySymbol, isCreateExpense);
        }
        set({
            participantsByCategory: finalParticipants,
            payersByCategory: finalPayers,
    ...
    validateForm: (options = { showErrors: true }) => {
        const { amount, participantsCategory, payersCategory, currencySymbol, participantsByCategory, payersByCategory, } = get();
        const { isValid, errors: validationErrors, amountErr, participantsErr, payersErr, showAmountErr, } = validateExpenseState(amount, participantsCategory.peek(), payersCategory.peek(), currencySymbol, participantsByCategory, payersByCategory);
        if (options.showErrors) {
            set((s) => ({
                amountError: s.amountError === amountErr ? s.amountError : amountErr,
            }));
            set((s) => ({
                showAmountError: s.showAmountError === showAmountErr
                    ? s.showAmountError
                    : showAmountErr,
            }));
            set((s) => ({
                participantsError: s.participantsError === participantsErr
                    ? s.participantsError
                    : participantsErr,
            }));
            set((s) => ({
    ...
    submitExpense: (inputs) => {
        ...
        submitExpenseFromState({
            ...
            ...inputs,
            fromPersonal: inputs.fromPersonal?.peek() || inputs.isPersonal?.peek() || undefined,
        });
    },
}));
