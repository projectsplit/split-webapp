  ...
  const handleNavigate = () => {
    if (selectedExpense.value?.transactionType === TransactionType.Group) {
      const jumpToken = createJumpToken(
        selectedExpense.value.occurred,
        selectedExpense.value.created
      );
      localStorage.removeItem(
        getFilterStorageKey('expense', selectedExpense.value.groupId)
      );
      navigate(
        `/shared/${selectedExpense.value.groupId}/expenses?jumpTo=${jumpToken}`
      );
    } else if (
