...
export default function DetailedExpense({
  ...
  const goToOriginExpense = () => {
    ...
    if (expense.transactionType === TransactionType.Group) {
      ...
      navigate(`/shared/${expense.groupId}/expenses?jumpTo=${jumpToken}`);
  ...
  const originScope =
    expenseType === TransactionType.Group ? 'group' : 'non group';
      ...
      <StyledDetailedExpense>
        ...
        {isEditable ? (
          ...
          <div className="footer navigateFooter">
            <span className="info">
              {`This is your share from a ${originScope} expense`}
            </span>
            <MyButton onClick={goToOriginExpense}>
              {`Go to ${originScope} expense`}
            </MyButton>
          </div>
        ) : null}
