...
export const currencyMask = (e, ticker, oldDisplayed, allowNegative) => {
    ...
    if (originalValue.length === oldDisplayed.length + 1) {
        ...
        if (added && added.char === ',') {
            processingValue =
                oldDisplayed.slice(0, added.pos) + '.' + oldDisplayed.slice(added.pos);
            isReplaced = true;
            addedPos = added.pos;
        }
    }
    ...
    const newValueInt = value.includes('.')
    ...
    if (cleanValue.includes('.') && unsignedCursorPosition > cleanIntLength) {
        newCursorPosition =
            cleanIntLength + newCommas + (unsignedCursorPosition - cleanIntLength);
    }
...
export function findAddedChar(oldStr, newStr) {
    if (newStr.length !== oldStr.length + 1)
        return null;
    for (let i = 0; i < oldStr.length; i++) {
        if (oldStr[i] !== newStr[i])
            return { pos: i, char: newStr[i] };
    }
